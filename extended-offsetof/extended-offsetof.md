<table>
<tr><td>Document number:</td><td>Dnnnn=yy-nnnn</td></tr>
...
### Convert structures to standard layout
...
#### Isn't standard-layout classes required for inter-process communication anyway?
...
### Use serialization for data exchange
...
<ins><br/>
<i>[Note:</i> The above restrictions apply to any containing classes or arrays thereof, starting with the class identified by <i>type</i> and including types of any intermediate members accessed by <i>member-designator</i> but not including the final member identified by <i>member-designator</i>. <i>&mdash; end note]</i></ins><br/>
Use of the <tt>offsetof</tt> macro with <del>a type</del><ins>types</ins> other than <del>a standard-layout class (Clause 9)</del><ins>those meeting the restrictions above</ins> is conditionally-supported. The expression <tt>offsetof</tt>(<i>type</i>, <i>member-designator</i>) is never type-dependent (14.6.2.2) and it is value-dependent (14.6.2.3) if and only if <i>type</i> is dependent. The result of applying the <tt>offsetof</tt> macro to a static data member or a function member is undefined.<ins> If <i>member-designator</i> accesses a reference data member or identifies a reference data member, or dereferences a pointer data member, the result of the <tt>offsetof</tt> macro is undefined.</ins> No operation invoked by the <tt>offsetof</tt> macro shall throw an exception and <tt>noexcept(offsetof(type, member-designator))</tt> shall be <tt>true</tt>.<ins><br/>
